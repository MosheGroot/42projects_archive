...
void    *thread_daemon_entry(void *data)
{
    ...
    while ((daemon_data->global_setup->num_of_prepared_philos
            != daemon_data->global_setup->num_of_philos)
        && daemon_data->global_setup->simulation_status != -1)
        usleep(25);
    if (daemon_data->global_setup->simulation_status != -1)
    {
...
void    thread_daemon(t_daemon *data)
{
    ...
    while (!data->target->is_dead && data->global_setup->simulation_status < 2)
    {
        ...
        {
            if (data->global_setup->simulation_status == 2)
                break ;
            data->global_setup->simulation_status = 2;
