...
void    print_event(const char *event, int event_id, t_philosopher *target)
{
    ...
    if (event_id != EVID_DIED && target->global_setup->simulation_status == 2)
        return ;
    pthread_mutex_lock(&target->global_setup->print);
    ...
    pthread_mutex_unlock(&target->global_setup->print);
}
